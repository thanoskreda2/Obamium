...
package net.mcreator.allahmod.world.biome;
...
public class NiggaBiomeBiome extends AllahModModElements.ModElement {
	@ObjectHolder("allah_mod:nigga_biome")
	public static final CustomBiome biome = null;
	public NiggaBiomeBiome(AllahModModElements instance) {
		super(instance, 8);
	}
	...
	static class CustomBiome extends Biome {
		public CustomBiome() {
			super(new Biome.Builder().downfall(0.5f).depth(0.1f).scale(0.2f).temperature(0.5f).precipitation(Biome.RainType.RAIN)
					.category(Biome.Category.NETHER).waterColor(-3355648).waterFogColor(-13382656)
					.surfaceBuilder(SurfaceBuilder.DEFAULT, new SurfaceBuilderConfig(ObamiumBlock.block.getDefaultState(),
							ObamiumBlock.block.getDefaultState(), ObamiumBlock.block.getDefaultState())));
			setRegistryName("nigga_biome");
			...
			addFeature(GenerationStage.Decoration.VEGETAL_DECORATION,
					new CustomTreeFeature()
							.withConfiguration((new BaseTreeFeatureConfig.Builder(new SimpleBlockStateProvider(ObamiumBlock.block.getDefaultState()),
									new SimpleBlockStateProvider(Blocks.DIAMOND_ORE.getDefaultState()))).baseHeight(8)
	...
	static class CustomTreeFeature extends AbstractTreeFeature<BaseTreeFeatureConfig> {
		...
		protected boolean place(IWorldGenerationReader worldgen, Random rand, BlockPos position, Set<BlockPos> changedBlocks,
			...
			if (position.getY() >= 1 && position.getY() + height + 1 <= world.getHeight()) {
				...
				if (!spawnTree) {
					...
					if (!((ground == ObamiumBlock.block.getDefaultState().getBlock() || ground == ObamiumBlock.block.getDefaultState().getBlock())
							&& (ground2 == ObamiumBlock.block.getDefaultState().getBlock()
									|| ground2 == ObamiumBlock.block.getDefaultState().getBlock())))
						return false;
					BlockState state = world.getBlockState(position.down());
					if (position.getY() < world.getHeight() - height - 1) {
						setTreeBlockState(changedBlocks, world, position.down(), ObamiumBlock.block.getDefaultState(), bbox);
								...
								for (int i2 = position.getZ() - j1; i2 <= position.getZ() + j1; ++i2) {
									...
									if (Math.abs(position.getX()) != j1 || Math.abs(j2) != j1 || rand.nextInt(2) != 0 && i4 != 0) {
										...
										if (state.getBlock().isAir(state, world, blockpos) || state.getMaterial().blocksMovement()
												|| state.isIn(BlockTags.LEAVES) || state.getBlock() == ObamiumBlock.block.getDefaultState().getBlock()
						...
						for (int genh = 0; genh < height; genh++) {
							...
							setTreeBlockState(changedBlocks, world, genhPos, ObamiumBlock.block.getDefaultState(), bbox);
							if (state.getBlock().isAir(state, world, genhPos) || state.getMaterial().blocksMovement() || state.isIn(BlockTags.LEAVES)
									|| state.getBlock() == ObamiumBlock.block.getDefaultState().getBlock()
									|| state.getBlock() == Blocks.DIAMOND_ORE.getDefaultState().getBlock()) {
								if (genh > 0) {
									if (rand.nextInt(3) > 0 && world.isAirBlock(position.add(-1, genh, 0)))
										setTreeBlockState(changedBlocks, world, position.add(-1, genh, 0), ObamiumBlock.block.getDefaultState(),
												bbox);
									if (rand.nextInt(3) > 0 && world.isAirBlock(position.add(1, genh, 0)))
										setTreeBlockState(changedBlocks, world, position.add(1, genh, 0), ObamiumBlock.block.getDefaultState(), bbox);
									if (rand.nextInt(3) > 0 && world.isAirBlock(position.add(0, genh, -1)))
										setTreeBlockState(changedBlocks, world, position.add(0, genh, -1), ObamiumBlock.block.getDefaultState(),
												bbox);
									if (rand.nextInt(3) > 0 && world.isAirBlock(position.add(0, genh, 1)))
										setTreeBlockState(changedBlocks, world, position.add(0, genh, 1), ObamiumBlock.block.getDefaultState(), bbox);
								}
							}
						}
						for (int genh = position.getY() - 3 + height; genh <= position.getY() + height; genh++) {
							...
							for (int genx = position.getX() - k4; genx <= position.getX() + k4; genx++) {
								for (int genz = position.getZ() - k4; genz <= position.getZ() + k4; genz++) {
									...
									if (state.isIn(BlockTags.LEAVES) || state.getBlock() == Blocks.DIAMOND_ORE.getDefaultState().getBlock()) {
		...
		private void addVines(IWorld world, BlockPos pos, Set<BlockPos> changedBlocks, MutableBoundingBox bbox) {
			setTreeBlockState(changedBlocks, world, pos, ObamiumBlock.block.getDefaultState(), bbox);
			int i = 5;
			for (BlockPos blockpos = pos.down(); world.isAirBlock(blockpos) && i > 0; --i) {
				setTreeBlockState(changedBlocks, world, blockpos, ObamiumBlock.block.getDefaultState(), bbox);
				blockpos = blockpos.down();
			}
		}

		private boolean canGrowInto(Block blockType) {
			return blockType.getDefaultState().getMaterial() == Material.AIR || blockType == ObamiumBlock.block.getDefaultState().getBlock()
					|| blockType == Blocks.DIAMOND_ORE.getDefaultState().getBlock() || blockType == ObamiumBlock.block.getDefaultState().getBlock()
					|| blockType == ObamiumBlock.block.getDefaultState().getBlock();
		}
